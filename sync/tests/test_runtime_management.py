...
class TestRuntimeManagement(unittest.TestCase):
	...
	def test_sync_context_exposes_delta_and_full_sync_state(self):
		...
		context = models_service.SyncContext(config=config, dry_run=False, last_successful_sync=last_sync)
	...
	def test_run_engine_partner_to_frappe_full_sync_passes_complete_partner_load_once(self):
		...
		with (
			patch(
				...
				return_value=SimpleNamespace(name="PARTNER-1"),
			),
			patch_service_dependency(
				"get_connector_for_partner",
				return_value=SimpleNamespace(ping=lambda: SimpleNamespace(ok=True, message="ok")),
			),
			patch("sync.sync.service.mapping._build_runtime_mapping_context", return_value=mapping_context),
			patch(
				"sync.sync.service.execution.sources._get_frappe_source_records", return_value=frappe_records
			),
			patch(
				"sync.sync.service.execution.sources._iter_partner_source_batches",
				return_value=iter(partner_batches),
			),
			patch("sync.sync.service.execution.one_way._sync_partner_to_frappe") as mock_sync,
		):
		...
		self.assertIsInstance(call["frappe_lookup"], models_service.FrappeMatchLookup)
	...
	def test_list_due_sync_definitions_uses_next_run_and_cron(self):
		...
		with (
			patch("sync.sync.service.orchestrator.frappe.get_meta", return_value=meta),
			patch(
			...
			patch(
				"sync.sync.service.orchestrator.frappe.get_doc",
				side_effect=AssertionError("get_doc should not be used"),
			),
			patch("sync.sync.service.scheduling._is_due_by_cron", side_effect=[True]),
		):
			self.assertEqual(scheduler_service.list_due_sync_definitions(now=now), ["READY", "CRON"])
	...
	def test_run_due_sync_definitions_limits_and_delegates(self):
		with (
			...
			patch(
				...
				side_effect=lambda name, **kwargs: {"sync_definition": name, **kwargs},
			) as mock_enqueue,
		):
	...
	def test_recover_stale_runs_marks_old_active_runs_and_updates_definition(self):
		...
		with (
			...
			patch(
				...
				return_value=SimpleNamespace(stale_run_timeout_minutes=60),
			),
			patch(
				"sync.sync.service.orchestrator.frappe.get_all",
				return_value=[
			...
			patch("sync.sync.service.orchestrator.frappe.get_doc", side_effect=fake_get_doc),
			patch("sync.sync.service.orchestrator.frappe.get_meta", side_effect=fake_meta),
			patch("sync.sync.service.orchestrator.frappe.db", _db_stub(exists=lambda *args, **kwargs: True)),
		):
			result = management_service.recover_stale_runs("SYNC-1")
	...
	def test_recover_stale_runs_uses_recent_run_activity(self):
		...
		with (
			...
			patch("sync.sync.service.orchestrator.frappe.get_doc") as mock_get_doc,
	...
	def test_enqueue_sync_definition_executes_immediately_when_queue_disabled(self):
		...
		with (
			...
			patch("sync.sync.service.orchestrator.frappe.get_doc", return_value=sync_definition),
	...
	def test_enqueue_sync_definition_uses_stale_period_as_long_queue_timeout(self):
		...
		with (
			...
			patch(
				"sync.sync.service.audit._get_sync_settings",
				return_value=SimpleNamespace(stale_run_timeout_minutes=45),
			),
			patch("sync.sync.service.orchestrator.frappe.generate_hash", return_value="abcdef12"),
			patch("sync.sync.service.orchestrator.frappe.enqueue") as mock_enqueue,
		):
			orchestrator_service.enqueue_sync_definition("SYNC-1")
	...
	def test_audit_flush_updates_run_activity_before_commit(self):
		...
		commit = Mock()
		run_doc = SimpleNamespace(name="RUN-1")
		audit_service._track_pending_run_writes(run_doc, 2)
		with (
			patch_service_dependency("now_datetime", return_value=datetime(2026, 3, 17, 12, 0, 0)),
			patch("sync.sync.service.orchestrator.frappe.db", _db_stub(set_value=set_value, commit=commit)),
		):
	...
	def test_preview_sync_definition_loads_document_and_predicts(self):
		...
		with (
			patch("sync.sync.service.orchestrator.frappe.get_doc", return_value=definition),
			patch.object(
				orchestrator_service.SyncPreviewService, "predict", return_value={"preview": True}
			) as mock_predict,
		):
			result = orchestrator_service.preview_sync_definition("SYNC-1", limit=7)
	...
	def test_sync_preview_service_delegates_to_build_preview(self):
		...
		with patch(
			...
			result = orchestrator_service.SyncPreviewService.predict(definition, limit=11)
	...
	def test_build_preview_returns_sample_and_ping(self):
		...
		connector = SimpleNamespace(
			ping=lambda: SimpleNamespace(ok=True, message="ok", details={"driver": "x"})
		)
		...
		with (
			...
			patch(
				"sync.sync.service.metadata._doctype_has_field",
				side_effect=lambda _doctype, field: field != "missing",
			),
			patch(
				"sync.sync.service.orchestrator.frappe.get_all",
				return_value=[{"name": "TASK-1", "subject": "Hello"}],
			) as mock_get_all,
		):
			result = orchestrator_service._build_preview(SimpleNamespace(name="SYNC-1"), limit=3)

		self.assertEqual(result["frappe_records_sample_count"], 1)
		self.assertEqual(result["partner_ping"]["ok"], True)
		self.assertEqual(
			result["mapping"], {"subject": {"partner_field": "title", "direction": "Frappe -> Partner"}}
		)
	...
	def test_export_sync_definition_yaml_includes_partner_and_type_documents(self):
		...
		partner_type = DummyDoc(
			{"doctype": "Sync Partner Type", "name": "postgres"}, doctype="Sync Partner Type"
		)
		...
		with (
			...
			patch(
				...
				side_effect=lambda data, mask_credentials=False: {**data, "masked": mask_credentials},
			),
			patch_service_dependency("now_datetime", return_value=datetime(2026, 3, 17, 12, 0, 0)),
		):
			payload = yaml.safe_load(yaml_io_service.export_sync_definition_yaml("SYNC-1"))
	...
	def test_export_sync_definition_yaml_omits_runtime_state_fields(self):
		...
		for fieldname in models_service.SYNC_DEFINITION_RUNTIME_STATE_FIELDS:
	...
	def test_normalize_sync_definition_payload_omits_runtime_state_fields(self):
		...
		with patch_service_dependency("frappe", new=_runtime_frappe_stub(get_meta=lambda doctype: meta)):
			normalized = yaml_io_service._normalize_doc_payload("Sync Definition", payload)
	...
	def test_preview_import_sync_definition_yaml_rejects_invalid_yaml_and_top_level_type(self):
		invalid = yaml_io_service.preview_import_sync_definition_yaml(":\n  bad", overwrite=True)
		not_mapping = yaml_io_service.preview_import_sync_definition_yaml("- item", overwrite=False)
	...
	def test_preview_import_sync_definition_yaml_marks_invalid_payload_sections(self):
		...
		with patch(
			"sync.sync.service.yaml_io._normalize_doc_payload", return_value={"doctype": "Sync Definition"}
		):
			result = yaml_io_service.preview_import_sync_definition_yaml(payload, overwrite=False)
	...
	def test_preview_import_sync_definition_yaml_rejects_legacy_version(self):
		result = yaml_io_service.preview_import_sync_definition_yaml(
	...
	def test_import_sync_definition_yaml_only_imports_mapping_sections(self):
		...
		with (
			patch(
				"sync.sync.service.yaml_io.preview_import_sync_definition_yaml",
				return_value={"can_import": True},
			),
			patch(
				"sync.sync.service.yaml_io._upsert_document_from_payload", side_effect=["PARTNER-1", "SYNC-1"]
			) as mock_upsert,
		):
			result = yaml_io_service.import_sync_definition_yaml(payload, overwrite=True)

		self.assertEqual(
			result, {"ok": True, "documents": {"Sync Partner": "PARTNER-1", "Sync Definition": "SYNC-1"}}
		)
	...
	def test_execute_sync_definition_handles_existing_run_and_run_name_shortcuts(self):
		with (
			patch("sync.sync.service.orchestrator._definition_lock", return_value=nullcontext()),
			patch("sync.sync.service.audit._has_active_run", return_value=True),
		):
			result = orchestrator_service.execute_sync_definition("SYNC-1")
		...
		with (
			patch("sync.sync.service.orchestrator._definition_lock", return_value=nullcontext()),
			patch_service_dependency("now_datetime", return_value=datetime(2026, 3, 17, 12, 0, 0)),
			patch("sync.sync.service.orchestrator.frappe.get_doc", side_effect=[run_doc, definition]),
			patch("sync.sync.service.audit._update_doc_fields"),
			patch(
				"sync.sync.service.configuration._build_definition_config", side_effect=RuntimeError("boom")
			),
			patch("sync.sync.service.orchestrator.frappe.log_error"),
			patch(
				"sync.sync.service.orchestrator.frappe.get_traceback",
				return_value="Traceback\nRuntimeError: boom",
			),
			patch("sync.sync.service.audit._update_definition_failure"),
			self.assertRaises(RuntimeError),
		):
			orchestrator_service.execute_sync_definition("SYNC-1", run_name="RUN-1")
	...
	def test_execute_sync_definition_reconnects_and_persists_job_timeout_traceback(self):
		...
		with (
			...
			patch(
				"sync.sync.service.audit._update_doc_fields",
			...
			patch(
				...
				side_effect=orchestrator_service.JobTimeoutException("deadline"),
			),
			patch("sync.sync.service.orchestrator._reconnect_database_after_job_timeout") as mock_reconnect,
			patch("sync.sync.service.orchestrator.frappe.log_error"),
			patch(
				"sync.sync.service.orchestrator.frappe.get_traceback",
				return_value="Traceback\nJobTimeoutException: deadline",
			),
			patch("sync.sync.service.audit._update_definition_failure") as mock_definition_failure,
			patch("sync.sync.service.orchestrator.frappe.db", _db_stub()),
			self.assertRaises(orchestrator_service.JobTimeoutException),
		):
			orchestrator_service.execute_sync_definition("SYNC-1", run_name="RUN-1")
	...
	def test_run_engine_requires_context_or_config_and_coerces_plain_objects(self):
		...
		with patch(
			...
			return_value=SimpleNamespace(is_submittable=True, fields=[]),
		):
	...
	def test_legacy_sync_definition_payload_actions_convert_to_write_hooks(self):
		payload = yaml_io_service._sync_definition_payload_with_legacy_hooks(
			{
				"doctype": "Sync Definition",
				"name": "SYNC-1",
				"frappe_after_insert_action": "Submit",
	...
	def test_saved_docs_with_loaded_empty_hook_table_do_not_fall_back_to_legacy_actions(self):
		...
		with patch("sync.sync.service.metadata._get_child_rows_by_options", return_value=[]):
			self.assertEqual(configuration_service._get_frappe_write_hooks(doc), ())
	...
	def test_legacy_hook_fallback_still_applies_when_child_table_lookup_fails(self):
		...
		with patch(
			"sync.sync.service.metadata._get_child_rows_by_options", side_effect=RuntimeError("legacy schema")
		):
			hooks = configuration_service._get_frappe_write_hooks(doc)
	...
	def test_mapping_helpers_support_top_level_string_payloads_and_value_reversal(self):
		...
		with patch("sync.sync.service.metadata._get_child_rows_by_options", return_value=[]):
			mapping = configuration_service._get_field_mapping(doc)
			self.assertEqual(configuration_service._get_match_fields(doc), ["name", "external_id"])
			self.assertEqual(
				mapping,
				{
			...
			self.assertEqual(configuration_service._get_value_mapping(doc), {"status": {"Open": "1"}})
			self.assertEqual(
				configuration_service._get_value_mapping_fallbacks(doc),
				{
					"name": {"action": "keep_original", "value": None},
					"status": {"action": "null", "value": None},
					"subject": {"action": "keep_original", "value": None},
				},
			)
		self.assertEqual(
	...
	def test_value_mapping_fallbacks_apply_only_after_mapping_miss(self):
		...
		value_mapping = {"status": {"Open": "1"}}
		value_mapping_fallbacks = {"status": {"action": "fallback", "value": "Unknown"}}
	...
	def test_value_mapping_fallbacks_can_keep_original_or_write_null(self):
		...
		with (
			patch("sync.sync.service.orchestrator.frappe.get_meta", return_value=datetime_meta),
			patch("sync.sync.service.time_utils._site_time_zone", return_value="Europe/Berlin"),
		):
			self.assertEqual(
				mapping_service._map_partner_to_frappe(
	...
	def test_runtime_mapping_context_reuses_reverse_maps_and_datetime_fields(self):
		...
		with (
			patch(
				"sync.sync.service.metadata._get_frappe_datetime_fields", return_value={"scheduled_at"}
			) as mock_datetime_fields,
			patch("sync.sync.service.metadata._doctype_fieldnames", return_value={"status", "scheduled_at"}),
			patch("sync.sync.service.time_utils._site_time_zone", return_value="Europe/Berlin"),
		):
	...
	def test_helper_functions_cover_string_lookup_and_lock_behaviour(self):
		...
		parent_doc = SimpleNamespace(
			doctype="Sync Definition", get=lambda key, default=None: [row] if key == "rows" else default
		)
		lock_context = object()
		cache = SimpleNamespace(lock=lambda *_args, **_kwargs: lock_context)

		with (
			patch("sync.sync.service.orchestrator.frappe.get_meta", return_value=parent_meta),
			patch("sync.sync.service.orchestrator.frappe.cache", return_value=cache),
		):
			rows = metadata_service._get_child_rows_by_options(parent_doc, "Sync Key Field")
			lock = orchestrator_service._definition_lock("sync:lock:1")

		self.assertEqual(rows, [{"field_name": "subject"}])
		self.assertIs(lock, lock_context)
		self.assertEqual(values_service._parse_lines(" a \n\n b "), ["a", "b"])
		self.assertEqual(values_service._clean_string("  value  "), "value")
		self.assertEqual(values_service._first_value({"a": "", "b": "x"}, ["a", "b"]), "x")
		self.assertEqual(values_service._first_value_dict({"a": "", "b": "x"}, ["a", "b"]), "x")
	...
	def test_runtime_change_detection_helpers_cover_edge_cases(self):
		...
		with patch("sync.sync.service.time_utils._site_time_zone", return_value="Europe/Berlin"):
			self.assertEqual(
				time_utils_service._parse_datetime("2026-03-17T10:00:00+00:00"),
				datetime(2026, 3, 17, 11, 0),
			)
			self.assertEqual(
				time_utils_service._parse_datetime(
					"2026-03-17 10:00:00",
		...
		self.assertIsNone(time_utils_service._parse_datetime("not-a-date"))
		self.assertIsNone(time_utils_service._parse_datetime(None))

	def test_runtime_record_helpers_compact_and_trim_values(self):
		config = SimpleNamespace(match_fields=["name"], mapping={"name": "id"})

		self.assertEqual(
			audit_service._compact_record_key(config, frappe_record={"name": "TASK-1"}, partner_record=None),
			"name=TASK-1",
		)
		self.assertEqual(audit_service._compact_source_id(config, frappe_record={"name": "TASK-1"}), "TASK-1")
		self.assertEqual(
			audit_service._compact_target_id(config, partner_record={"id": "TASK-1"}), "id=TASK-1"
		)
		self.assertEqual(audit_service._fit_data_value("x" * 150), f"{'x' * 137}...")
	...
	def test_run_engine_aborts_before_delete_missing_when_partner_load_is_partial(self):
		...
		with (
			...
			with self.assertRaisesRegex(
				...
				engine_service._run_engine(
					SimpleNamespace(name="SYNC-P2F"), SimpleNamespace(name="RUN-1"), config=config
				)
	...
	def test_bidirectional_skips_when_no_differences_and_prefers_partner_when_only_partner_changed(self):
		...
		with (
			...
			bidirectional_service._sync_bidirectional(
				...
				connector=object(),
				frappe_records=[
					{"name": "TASK-1", "status": "Open", "modified": "2026-03-17 09:00:00"},
					{"name": "TASK-2", "status": "Closed", "modified": "2026-03-17 09:00:00"},
				],
				partner_records=[
					{"id": "TASK-1", "state": "Open", "updated_at": "2026-03-17 09:00:00"},
					{"id": "TASK-2", "state": "Open", "updated_at": "2026-03-17 11:00:00"},
				],
				dry_run=False,
				stats=models_service.SyncStats(),
	...
	def test_update_partner_connection_status_and_due_by_cron_helpers(self):
		...
		meta = DummyMeta(
			[_field("last_connection_status"), _field("last_checked_on"), _field("last_connection_error")]
		)
		...
		with (
			...
			management_service._update_partner_connection_status(partner_doc, status="error", details="boom")
		...
		with patch_service_dependency("croniter", None):
			self.assertFalse(
				scheduling_service._is_due_by_cron(SimpleNamespace(name="SYNC-1"), "*/5 * * * *", now)
			)

		with (
			patch_service_dependency("croniter", DummyCroniter(error=True)),
			patch("sync.sync.service.orchestrator.frappe.logger", return_value=cron_logger),
		):
			self.assertFalse(scheduling_service._is_due_by_cron(SimpleNamespace(name="SYNC-1"), "bad", now))

		with (
			patch_service_dependency("croniter", DummyCroniter(previous=datetime(2026, 3, 17, 11, 55, 0))),
			patch("sync.sync.service.orchestrator.frappe.get_meta", return_value=run_meta),
	...
	def test_runtime_document_helpers_create_and_update_records(self):
		...
		with (
			...
			created_run = audit_service._create_run_doc(
				SimpleNamespace(
					name="SYNC-1",
					get=lambda key, default=None: {
						"sync_type": "Frappe -> Partner",
			...
			created_item = audit_service._create_run_item(
				run_doc=SimpleNamespace(
					name="RUN-1",
					get=lambda key, default=None: {"sync_type": "Frappe -> Partner"}.get(key, default),
				),
				config=SimpleNamespace(match_fields=["name"], mapping={"name": "id"}),
	...
	def test_runtime_update_helpers_and_last_successful_sync(self):
		...
		with (
			...
			audit_service._update_doc_fields(
				SimpleNamespace(doctype="Sync Run", db_set=doc.db_set), {"status": "Success", "missing": "x"}
			)
			audit_service._update_definition_runtime(
				doc, last_run="RUN-1", last_sync_at=next_run, summary="ok"
			)
			audit_service._update_definition_failure(doc, last_run="RUN-2", error_message="line1\nline2")
		...
		with (
			patch_service_dependency("croniter", DummyCroniter(next_value=next_run)),
			patch_service_dependency("now_datetime", return_value=datetime(2026, 3, 17, 12, 0, 0)),
			patch_service_dependency(
				"frappe", new=_runtime_frappe_stub(get_meta=lambda *_args, **_kwargs: meta)
			),
		...
		runs = [
			{
				...
				"status": "Success",
				"dry_run": 1,
				"finished_at": "2026-03-17 12:00:00",
			},
			{
				"sync_definition": "SYNC-1",
				"status": "Success",
				"dry_run": 0,
				"finished_at": "2026-03-17 11:55:00",
			},
		]
		...
		with (
			patch(
				"sync.sync.service.orchestrator.frappe.get_meta",
				return_value=DummyMeta(
					[_field("sync_definition"), _field("status"), _field("dry_run"), _field("finished_at")]
				),
			),
			patch("sync.sync.service.orchestrator.frappe.get_all", side_effect=get_all) as mock_get_all,
		):
			self.assertEqual(
				audit_service._get_last_successful_sync("SYNC-1"), datetime(2026, 3, 17, 11, 55, 0)
			)

		self.assertEqual(mock_get_all.call_args.kwargs["filters"]["dry_run"], 0)

		self.assertIn(
			"processed=5", audit_service._format_run_summary({"processed_count": 5, "error_count": 1})
		)
	...
	def test_update_helpers_batch_valid_field_writes_and_preserve_commit_flags(self):
		...
		with patch_service_dependency(
			...
			audit_service._update_doc_fields(
				run_doc, {"status": "Success", "missing": "ignored"}, commit=True
			)
